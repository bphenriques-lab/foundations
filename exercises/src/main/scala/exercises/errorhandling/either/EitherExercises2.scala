...
object EitherExercises2 {
  ...
  def validateCountry(countryCode: String): Either[ValidationError, Country] =
    for {
      _ <-
        if (countryCode.length == 3 && countryCode.forall(char => char.isLetter && char.isUpper)) Right(countryCode)
        else Left(InvalidFormat(countryCode))
      country <- Country.all.find(_.code == countryCode).toRight(NotSupported(countryCode))
    } yield country
