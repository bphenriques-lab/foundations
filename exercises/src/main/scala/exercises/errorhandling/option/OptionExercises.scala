...
object OptionExercises {
  ...
  sealed trait Role {
    ...
    def getAccountId: Option[AccountId] = this match {
      case Reader(accountId, _) => Option(accountId)
      case Editor(accountId, _) => Option(accountId)
      case Role.Admin => Option.empty
    }
  }
  object Role {
  ...
  def sequence[A](options: List[Option[A]]): Option[List[A]] =
    options.foldLeft[Option[List[A]]](Option(List.empty)) { case (state, option) =>
