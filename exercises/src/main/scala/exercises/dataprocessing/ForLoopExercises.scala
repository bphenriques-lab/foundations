...
object ForLoopExercises {
  ...
  def sum(numbers: List[Int]): Int = pattern(numbers, 0) { (r, n) => r + n }

  // a. Implement `size` using a mutable state and a for loop
  // such as size(List(2,5,1,8)) == 4
  // and     size(Nil) == 0
  def size[A](items: List[A]): Int = pattern(items, 0) { (r, _) => r + 1 }
  ...
  def min(numbers: List[Int]): Option[Int] = pattern(numbers, Option.empty[Int]) { (r, n) =>
    r match {
  ...
  def wordCount(words: List[String]): Map[String, Int] = pattern(words, Map.empty[String, Int]) { (result, word) =>
  ...
  def pattern[T, R](list: List[T], initialResult: R)(combine: (R, T) => R): R = {
